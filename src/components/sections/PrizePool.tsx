  ...
  >
    ...
    >
      ...
      <div className="text-6xl font-black text-slate-800 tracking-tighter mb-6 flex items-start">
        <span className="text-xl font-bold text-indigo-500 mt-2 mr-1">₹</span>{amount}
      </div>
      <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 bg-slate-50/80 px-5 py-2.5 rounded-2xl border border-slate-100 group-hover:bg-indigo-500/10 group-hover:text-indigo-600 group-hover:border-indigo-500/20 transition-all duration-500">
        Cash Prize + Goodies
      </div>

      {/* Animated glow */}
      <div className="absolute -bottom-12 -right-12 w-32 h-32 bg-indigo-400/10 blur-[40px] rounded-full group-hover:bg-indigo-400/20 transition-colors" />
    </motion.div>
  </motion.div>
);
...
const PrizePool = () => (
  <section id="prizes" className="py-24 px-6 max-w-7xl mx-auto">
    <div className="max-w-7xl mx-auto">
      ...
      >
        <div className="inline-flex items-center gap-2 p-4 glass rounded-2xl border border-indigo-100 text-slate-600 hover:scale-105 transition-transform cursor-default">
          <ShieldCheck className="w-5 h-5 text-indigo-500" />
          <span className="font-bold uppercase tracking-tight text-xs">Certificates for all participants + Swags for Winners</span>
        </div>
      </motion.div>
    </div>
  </section>
